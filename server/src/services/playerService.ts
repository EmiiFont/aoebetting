...
export class PlayerService implements IPlayerService {
  ...
  async autoAddPlayers(top: number): Promise<Player[]> {
    const aoePlayers: ApiPlayer[] = await this._aoeApiService.getPlayers(top);
    const tournamentEloPlayers = await this._tournamentElo.getPlayers();
    const addedPlayers: Player[] = [];

    try {
      for (const tourEloPlayer of tournamentEloPlayers) {
        const aoe2EloPlayer = aoePlayers.filter(
          (v) => v.name.toLocaleLowerCase().indexOf(tourEloPlayer.name.toLocaleLowerCase()) >= 0,
        );

        if (aoe2EloPlayer.length > 0) {
          const aoeEloFullPlayer = await this._tournamentElo.getPlayerInfo(tourEloPlayer.id);
          ...
          if (steamId != undefined) {
            const tournamentElo = tourEloPlayer.elo;

            const averagedElo = (first.rating + tournamentElo) / 2;
