...
export class BetService implements IBetService {
  ...
  async setSystemBet(bet: BetDTO, matchUid: number): Promise<Array<Bet>> {
    ...
    eventStartingOdds.forEach((d) => {
      const systemBet = this._betRepository.create({
        type: betType,
        datePlaced: new Date(),
        systemOdd: d.odd,
        match: match,
  ...
  async setUserbet(userBet: BetDTO, betUid: number): Promise<boolean> {
    ...
    const currentBets: Array<BetDTO> = [];

    //second we get the eventOdd for both sides e.g player1 vs player2
    const eventOdd: Array<EventsOdd> = [];

    const balancedOdds = await this._bookie.setOdds(currentBets, eventOdd);

    const oddForPickedSide = balancedOdds.find((v) => v.eventId == betUid);

    if (oddForPickedSide != undefined) {
      //let user = await this._userRepository.findOne({ uid: userBet.user?.uid});
      const betTypeName = BetTypeEnum[userBet.betType ?? BetTypeEnum.MoneyLine];
      const bet = await this._betRepository.findOne({ uid: betUid });
