...
export class LiquipediaService implements IliquipediaService{
    ...
    async getMatchList(): Promise<ILiquipediMatchhList> {
            const matchesURL = `https://liquipedia.net/ageofempires/api.php?action=parse&origin=*&format=json&page=Liquipedia:Upcoming_and_ongoing_matches`;
    ...
    private _parseMatch(matchSet: cheerio.Cheerio, isLive = false): ILiquipediMatch[] {
        ...
        for (let i = 0, len = matches.length; i < len; i++) {
            ...
            const matchObj: ILiquipediMatch = {
                startTime: isLive ? undefined : this._parseTime(match.find('.match-filler').find('.match-countdown').find('span > span').text().trim()),
                team1: this.parseTeamSides(match, true),
                team1Score: isLive ? this._parseScore(match.find('.versus').eq(0).text()) : this._parseScore(match.find('.versus').eq(0).text()),
                team2: this.parseTeamSides(match, false),
                team2Score: isLive ? this._parseScore(match.find('.versus').eq(0).text(), true) : this._parseScore(match.find('.versus').eq(0).text(), true),
                tournament: match.find('.match-filler').find('.match-countdown + div > div > a').eq(0).attr('title') || "",
                tournamentURL: `https://liquipedia.net/ageofempires/${match.find('.match-filler').find('.match-countdown + div > div > a').eq(0).attr('href')}`,
            };
    ...
    private parseTeamSides(match: cheerio.Cheerio, isLeft: boolean): string {
        if(isLeft){
           let teamName = match.find('.team-left .team-template-text').find('a').eq(0).attr('title') || "";
          ...
          if(teamName === ""){
            let alink =  match.find('.team-right').find('a');
