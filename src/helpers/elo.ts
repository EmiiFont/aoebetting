...
export class Elo {
    ...
    */
    ...
    {
        let newRatingsList: Array<number> =
        [
            this._newRatingA,
            this._newRatingB
        ]
    ...
    protected _getExpectedScores(ratingA: number, ratingB: number): Array<number>
    {
        ...
        let expectedScoresList: Array<number> =
        [
            expectedScoreA,
            expectedScoreB
        ];
     ...
     * @return Array<number>
     */
    protected _getNewRatings(ratingA: number, ratingB: number, expectedA: number, expectedB: number, scoreA: number, scoreB: number): Array<number>
    {
        let newRatingA = ratingA + (this.KFACTOR * ( scoreA - expectedA ) );
        let newRatingB = ratingB + (this.KFACTOR * ( scoreB - expectedB ) );

        let newRatingList: Array<number> =
        [
            newRatingA,
            newRatingB
        ];
